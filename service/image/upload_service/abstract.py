...
from marble.service.image.model import MarbleImage
from marble.rest.exceptions import InvalidParamTypeError, ServerError
...
class ImageUploadServiceAbstract(object):
    ...
    def __init__(self,
                 source_file,
                 thumb_size=settings.MARBLE_IMAGE_SIZE_THUMBNAIL,
                 small_size=settings.MARBLE_IMAGE_SIZE_SMALL,
                 middle_size=settings.MARBLE_IMAGE_SIZE_MEDIUM,
                 large_size=settings.MARBLE_IMAGE_SIZE_LARGE
                 ):
    ...
    def process_image(self, image_class=MarbleImage):
        ...
        :param image_class: Class object for creating instance, should be subclass of ``MarbleImage``.
         If not provided ``MarbleImage`` used by default.
        :type image_class: class
        :return: Instance of processed ``MarbleImage`` (or subclass if class provided) with filled URLs for all resolutions
        :rtype: MarbleImage
