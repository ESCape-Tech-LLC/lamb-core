...
from marble.rest.exceptions import ServerError, ExternalServiceError
from marble.service.image.upload_service.abstract import ImageUploadServiceAbstract
...
class ImageUploadServiceAmazonS3(ImageUploadServiceAbstract):
    ...
    def __init__(self, bucket_name, *args, **kwargs):
        ...
        session = Session(
            aws_access_key_id=settings.MARBLE_AWS_ACCESS_KEY,
            aws_secret_access_key=settings.MARBLE_AWS_SECRET_KEY
        )
    ...
    def store_image(self, image, suffix=None, proposed_name=None):
        ...
        temp_file = os.path.join(settings.MARBLE_STATIC_FOLDER, file_name)
        image.save(temp_file, 'JPEG', quality=settings.MARBLE_IMAGE_UPLOAD_QUALITY)
