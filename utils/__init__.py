...
from marble.rest.exceptions import InvalidBodyStructureError, InvalidParamTypeError, InvalidParamValueError, ServerError

class PYNMRequest(HttpRequest):
    """ Class used only for proper type hinting in pycharm, not guarantee that properties will exist
    :type marble_db_session: sqlalchemy.orm.Session | None
    :type marble_execution_meter: pynm.execution_time.ExecutionTimeMeter | None
    """
    def __init__(self):
        super(PYNMRequest, self).__init__()
        self.marble_db_session = None
        self.marble_execution_meter = None
...
def paginated(data, request):
    ...
    limit = dpath_value(request.GET, 'limit', int, default=settings.MARBLE_PAGINATION_WINDOW)
    if limit < 0:
        raise InvalidParamValueError('Invalid limit value for pagination', details='limit')
    if limit > settings.MARBLE_PAGINATION_WINDOW:
        limit = settings.MARBLE_PAGINATION_WINDOW
