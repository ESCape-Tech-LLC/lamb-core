...
from marble.json import JsonResponse
from marble.rest.exceptions import *

apply_to_apps = getattr(settings, 'MARBLE_REST_APPLIED_APPS', [])

logger = logging.getLogger('django')

class NMRestApiJsonMiddleware(object):
    ...
    def process_response(self, request, response):
        ...
        _ = request.POST
        _ = request.FILES
        logger.debug('NMRestApiJsonMiddleware. Processing response: %s' % response)
        try:
    ...
    def process_exception(self, request, exception):
        ...
        """
        logger.debug('NMRestApiJsonMiddleware. Processing exception: %s' % exception)
        ...
        if isinstance(exception, (SQLAlchemyError, DBAPIError)):
            status_code = 500
            error_code = PYNM_REST_APP_ERROR_DATABASE
            ...
            status_code = 500
            error_code = PYNM_REST_APP_ERROR_UNKNOWN
