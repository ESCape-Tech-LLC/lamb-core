...
def _redis_rate_parse_limits(bucket_name_base, limits: List[Tuple[int, int]]) -> List[RateLimit]:
    ...
    for duration, limit in limits_dict.items():
        ...
        rate_limit = RateLimit(key=bucket_name_slot, limit=limit, current=0, duration=duration, time_slot=time_slot)
...
def _redis_rate_parse_response(response: Dict[str, dict]) -> List[RateLimit]:
    ...
    for key, res in response.items():
        ...
        rate_limit = RateLimit(key=key, limit=limit, current=current, duration=duration, time_slot=time_slot)
        result.append(rate_limit)
...
def redis_rate_check_lua(
    conn: redis.Redis, bucket_name_base: str, limits: List[Tuple[int, int]] = list(), increment: bool = True
) -> List[RateLimit]:
    # check and sort limits
