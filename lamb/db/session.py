...
try:
    ...
    logger.info(
        f"database.  SYNC options[pooled =  True]: engine_opts={ENGINE_OPTS_POOLED}" f", session_opts={_SESSION_OPTS}"
    )
    ...
    if ENGINE_OPTS_NON_POOLED is None:
        ENGINE_OPTS_NON_POOLED = {}
    logger.info(
        f"database.  SYNC options[pooled = False]: engine_opts={ENGINE_OPTS_NON_POOLED}"
        f", session_opts={_SESSION_OPTS}"
    )
    ...
    raise ServerError("Database session constructor failed to get database params") from e
...
if _ENGINE == "postgresql":
    ...
    if ASYNC_ENGINE_OPTS_POOLED is None:
        ASYNC_ENGINE_OPTS_POOLED = {"pool_recycle": 3600, "pool_size": 50, "max_overflow": 50}
    if ASYNC_ENGINE_OPTS_POOLED is None:
...
logger.info(
    f"database. ASYNC options[pooled =  True]: engine_opts={ASYNC_ENGINE_OPTS_POOLED}" f", session_opts={_SESSION_OPTS}"
)

logger.info(
    f"database. ASYNC options[pooled = False]: engine_opts={ASYNC_ENGINE_OPTS_NON_POOLED}"
    f", session_opts={_SESSION_OPTS}"
)
